    ...
    <form class="form-horizontal" action="{{ route('userRole.permissionUpdate') }}" method="POST" enctype="multipart/form-data" id="editUser" name="editUser">
        ...
        @endphp
            <div class="row parentMenuBlock">
                <div class="col-md-12">
                    <label>
                        <input class="parentMenu_{{$mainMenu->parentMenu}} menu" type="checkbox" name="usermenu[]" value="{{$mainMenu->id}}" {{$checked}}  data-id="{{$mainMenu->id}}" @if($mainMenu->id == 3) onclick="return false" @endif>
