...
<div class="row">
    <div class="col-md-3">
        <h5><strong>Search By Parent Menu</strong></h5>
        <form>
            <select name="parentMenuParam" class="form-control chosen-select parentMenu">
                    <option value="">View All</option>
                @foreach ($parentMenuList as $parentMenu)
                   @php
                        if(@$parentMenuParam){
                            if($parentMenu->id == $parentMenuParam){
                                $selected = 'selected';
                            }else{
                                $selected = '';
                            }
                        }
                        $getMenu = Menu::where('parent',$parentMenu->id)->get();
                        if($parentMenu->parent == NULL OR count($getMenu) > 0){
                   @endphp
                    <option {{@$selected}} value="{{$parentMenu->id}}">{{$parentMenu->articleName}}
                    </option>
                    @php
                        }
                    @endphp
...
@section('custom-js')
    ...
    <script type="text/javascript">
        $('.parentMenu').on('change', function(){
          ...
          window.location.href = "{{route('menu.index')}}"+"?parentMenu="+parentMenu;
        }); 
    </script>
@endsection