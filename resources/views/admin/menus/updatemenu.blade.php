...
<form class="form-horizontal" action="{{ route('menu.edit',$menu->id) }}" method="POST" enctype="multipart/form-data" id="editMenu" name="editMenu">
    ...
    <div class="col-sm-9">
        <input type="text" class="form-control form-control-danger" placeholder="header name" name="articleName" value="{{ $menu->articleName }}" required="">
...
<div class="form-group row {{ $errors->has('firstHomeImage') ? ' has-danger' : '' }}">
    ...
    <div class="col-sm-9">
        ...
        <?php if (file_exists(@$menu->firstHomeImage)) { ?> 
            <img src="{{asset('/').@$menu->firstHomeImage}}" style="height: 80px;">
        <?php }else{ ?>
            <img src="{{$noImage}}" style="height: 80px;">
        <?php } ?>
    ...
    <div class="form-group row {{ $errors->has('showInMenu') ? ' has-danger' : '' }}">
        ...
        <div class="col-sm-9">
            <div class="form-control">
                ...
                <div class="custom-control custom-radio">
                    <input type="radio" id="hideInMainMenu" name="showInMenu" class="custom-control-input" value="0">
