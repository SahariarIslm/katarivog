...
class MenuController extends Controller
{
    ...
    {   $title = "Manage Menu";
        $parentMenuParam = @$_GET['parentMenu'];
        $parentMenuList = Menu::all(); 
        if($parentMenuParam){
            $menus = Menu::where('parent',$parentMenuParam)->orWhere('id',$parentMenuParam)->get(); 
        }else{
            $menus = Menu::orderBy('id',"ASC")->get();
        }
        return view('admin.menus.index')->with(compact('title','parentMenuParam','parentMenuList','menus'));
    }

    public function add(Request $request){
        $title = "Add New Menu";
        $menus = Menu::orderBy('id',"ASC")->where('parent',NULL)->get();
        ...
        if (isset($request->firstHomeImage)) {
            $firstHomeImage = \App\HelperClass::UploadImage($request->firstHomeImage,'menus','images/articles/home/');
        }

        if (isset($request->firstHomeImage)) {
            $firstHomeImage = \App\HelperClass::UploadImage($request->firstHomeImage,'menus','images/menus/','2000','585');
        }
        ...
        $menu = Menu::create( [
           ...
           return redirect(route('menu.index',['parentMenu'=>$request->parent]))->with('msg','Menu Created Successfully');
        }else{
    ...
    public function edit(Request $request, $id=NULL){
        ...
        if(count($request->all()) > 0){
         ...
         $menu->update( [
           ...
           return redirect(route('menu.index',['parentMenu'=>$request->parent]))->with('msg','Menu Updated Successfully');
        }else{
